    ...
    final override func PostProcessing(for result: Result<Bool, ScanFailReason>, done: @escaping () -> Void) {
        ...
        switch result {
            ...
            DispatchQueue.main.async {
                ...
                guard let scannedRect = RectangleDetectionHandler.detectRectangle(image: image!) else { return }
                
                
                let imageProcessViewController = ImageProcessViewController(image: image!.withHorizontallyFlippedOrientation(), rect: scannedRect) { (originalImage, croppedImage) in
